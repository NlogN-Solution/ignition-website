...
import { ArrowButton, GhostButton } from "./ArrowButton";
import { loginUrl, navItems } from "@/lib/navigation";
  ...
  return (
    ...
    >
      <div className="mx-auto grid h-[68px] grid-cols-[auto_1fr_auto] items-center gap-4 px-5 sm:px-8 lg:grid-cols-[1fr_auto_1fr] xl:h-[90px] xl:px-12">
          ...
          <ul className="flex items-center gap-[45px]">
            {navItems.map((item) => (
              <li key={item.label} className="group relative">
                ...
                <div className="invisible absolute left-1/2 top-full z-10 w-56 -translate-x-1/2 translate-y-1 rounded-xl border border-hairline bg-white p-2 opacity-0 shadow-[0_24px_48px_-24px_rgba(1,22,111,0.35)] transition-all duration-200 group-focus-within:visible group-focus-within:translate-y-0 group-focus-within:opacity-100 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100">
                  {item.items.map((sub) => (
                    <Link
                      key={sub.label}
                      href={sub.href}
        ...
        <div className="hidden items-center gap-[15px] justify-self-end lg:flex">
          <GhostButton href={loginUrl} className="h-[39px] px-[22px] text-[14px]">
            Login
          </GhostButton>
          <ArrowButton
            href="/start"
      ...
      {open ? (
        <div className="border-t border-hairline bg-white px-5 pb-6 pt-2 sm:px-8 lg:hidden">
          <ul className="divide-y divide-hairline">
            {navItems.map((item) => (
              <li key={item.label} className="py-3">
                <p className="mb-1 text-[15px] font-bold text-navy">
                  {item.label}
                </p>
          ...
          <div className="mt-5 flex items-center gap-3">
            <GhostButton
              href={loginUrl}
              className="h-[44px] flex-1 px-5 text-[15px]"
            >
              Login
            </GhostButton>
            <ArrowButton
